...
class SkillExecutor:
    """
    Defines the execution interface for all skills in the Client Assessment chain.

    Skills 1, 2, 5, 6 are wired via the Option C adapter layer (delegating to
    existing certified executors in other runtimes). Skills 3, 4, and FM use
    CA-local net-new executors wrapped by their respective adapters.

    Tests may override individual methods with lambda fixtures (the adapter
    ...
    def execute_skill_fm(self, state_mgr, inputs: dict, logger) -> dict:
        """Skill FM: ethana-feature-mapping. Returns feature_mapping_output JSON."""
        return self.adapters["fm"].execute(state_mgr, inputs, logger)
